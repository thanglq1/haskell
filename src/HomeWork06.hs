...
-- Pass orderList and bevogBeerPrices then result is [("Tak", 30.0),("Kramah", 28.0),("Ond", 59.5)]
costHelper :: [(String, Double)] -> [(String, Double)] -> [(String, Double)]
costHelper      [] _                          = []
costHelper      _ []                          = []
costHelper      ((x1,x2):xs) ((y1,y2):ys)   = if (x1 == y1)
                                              then [(x1, x2 * y2)] ++ (costHelper xs ys)
                                              else []
-- Sum 1 array tupple [("Tak", 30.0),("Kramah", 28.0),("Ond", 59.5)] = 30 + 28 + 59.5
sumTuple :: [(String, Double)] -> Double
sumTuple    []          = 0
sumTuple    ((x, y):xs) = y + sumTuple xs


beerCost :: [(String, Double)] -> Double
beerCost x = deliveryCost + (sumTuple (costHelper x bevogBeerPrices))


-- define again some function
and' :: [Bool] -> Bool
and'    []     = True
and'    (x:xs) = x && and' xs
...
drop' :: Int -> [a] -> [a]
drop'     n x         | n <= 0         = x
drop'     n x         | n > length x   = x
drop'     n (_:xs)                     = drop' (n-1) xs

take' :: Int -> [a] -> [a]
take'     n _         | n <= 0         = []
take'     n x         | n > length x   = x
