...
bordeauxFrance :: Region
bordeauxFrance = ("France", "Bordeaux")
...
data WineType = RED | WHITE | ROSE deriving Enum
data Kind' = Kind' { kind :: WineType, level :: Float }
k2 :: Kind'
k2 = Kind' { kind = RED, level = 15.5}
...
data Label = Label { grapes :: [Grape], kind' :: Kind, region' :: Region } deriving (Show)

-- Larrosa Rose is a rose wine from the region Rioja. It is made from the Garnacha grape and 
-- has a alcohol level of 14%.
larrosaRose :: Label
larrosaRose = Label { grapes = ["Garnacha"], kind' = Rose 14, region' = ("France", "Rioja") }
...
castiglioni :: Label
castiglioni = Label { grapes = ["Sangiovese"], kind' = Red 12.5, region' = ("France", "Tuscany") }
...
bordeaux :: Label
bordeaux = Label { grapes = ["Cabernet-sauvignon", "Merlot"], kind' = Red 13.5, region' = ("France", "Bordeaux") }
...
-- newGrape = "Pinot Noir"
containsGrape :: [Grape] -> Grape -> Bool
containsGrape grapes grape = grape `elem` grapes